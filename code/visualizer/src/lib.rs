...
impl From<Point2d> for SerializedPoint2d {
  fn from(value: Point2d) -> Self { Self { x: value.x, y: value.y } }
}
  ...
  fn from(value: GeoAdjacencyMatrix) -> Self {
    Self(
      value
        ...
        .into_iter()
        .map(|row_vec| {
          row_vec.into_iter().map(SerializedGeoEdge::from).collect()
        })
        .collect(),
    )
  }
...
pub fn triangulate(points: &[u8]) -> Result<Vec<u8>> {
  let (points, adjacency_matrix) = init(
    &ciborium::from_reader::<Vec<_>, _>(points).map_err(|e| e.to_string())?,
  )
  .ok_or(String::from(
    "some point is not denoted by a key-value pair of x and y coordinates",
  ))?;
  let adjacency_matrix =
    SeralizedGeoAdjacencyMatrix::from(adjacency_matrix.triangulate(points));
  let mut buf = Vec::new();

  ciborium::into_writer(&adjacency_matrix, &mut buf)
...
fn init(
  raw_points: &[HashMap<String, f64>],
) -> Option<(Vec<Point2d>, GeoAdjacencyMatrix)> {
  Some(GeoAdjacencyMatrix::from_point_set(raw_points.iter().try_fold(
