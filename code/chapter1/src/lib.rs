...
#![feature(
  ...
  iter_order_by, try_with_capacity, allocator_api
)]
...
impl Pairs<'_> {
  ...
  pub fn ancestors(&self, this: usize) -> Result<Vec<usize>, PairsError> {
    let (this_root, mut parent, mut ancestors) =
      (self.find(this)?, this, vec![this]);
  ...
  pub fn cartesian_product(&mut self) {
    ...
    self.current_product =
      iter::repeat_n(self.current_node.expect(&ERROR_MSG), others.len())
        .zip(others)
        .collect();
  }
      ...
      stack:        {
        ...
        output.push(
          self.forest[root.next().expect(
            "this should yield the tree root sourced directly from `self`",
          )],
        );
...
impl GeoAdjacencyMatrix {
  ...
  pub fn build_hull(
    ...
    for &(vertex, point) in points {
        ...
        }
      {
        ...
        let (&(prev, _), post) =
          (unsafe { hull.last().unwrap_unchecked() }, vertex);

        (
          triangulation[prev][rm], triangulation[post][rm],
          triangulation[rm][prev], triangulation[rm][post],
  ...
  pub fn optimize_triangulation(&self, triangulation: &mut [Vec<GeoEdge>]) {
    while let Some(((src, dst), (p1, p2))) = Self::find_next_opt(triangulation)
    {
      ...
      (
        ...
        triangulation[p1][p2], triangulation[p2][p1],
      ) = (
        GeoEdge::NonExistent,
        GeoEdge::NonExistent,
        self[p1][p2],
  ...
  ///
  /// To find an edge in the triangulation that is determined to be illegal, the
  /// following algorithm is used:
  ///
  /// + Try to find two other points that would make up a quadrilateral
  ///   alongside the edge at hand (denoted by some two points (`src`, `dst`),)
  ///   and then make sure the quadrilateral is convex.
  /// + At this point, we could fail at the start if the edge is not an inner
  ///   edge but rather a boundary edge of the convex hull, or we could fail in
  ///   finding a convex quadrilateral because one of the points in the
  ...
  pub fn find_next_opt(
    ...
    triangulation
      ...
      .enumerate()
      .flat_map(|(src, row)| {
        (0..row.len()).skip(src + 1).map(move |dst| (src, dst))
      })
      .find_map(|(src, dst)| {
        let (
  ...
  fn tsp(&self) -> Vec<usize> {
    ...
    while visited.iter().any(|visited| !visited) {
      ...
      (current_idx, _) = current
        ...
        .enumerate()
        .filter_map(|(idx, edge)| {
          (!visited[idx] && matches!(edge, Edge::Weighted(_)))
            .then_some((idx, edge))
        })
...
impl TspTriMstDfs for GeoAdjacencyMatrix {
  ...
  fn triangulate(&self, points: Self::PointList) -> Self::Triangulation {
    ...
    );
    {
      ...
      self.build_hull(
        ...
        |prev_last, last, point| {
          ...
          // O'Rourke, 2001.
          Self::compute_raw_triangle_area((prev_last, last, point)).signum()
            == -1.
        },
        &points,
      );
    ...
    let mut triangulate_bounds_of = |collection: Vec<(usize, Point2d)>| {
      collection.windows(2).map(|inner| (inner[0].0, inner[1].0)).for_each(
        |(src, dst)| {
...
mod tests {
  ...
  fn process_rows(
    inner: &[Value],
  ) -> Result<GeoAdjacencyMatrix, AdjacencyMatrixError> {
    let inner = inner.iter().try_fold(
      ...
      match value {
        | Value::Array(inner) => {
          match inner.first().ok_or(AdjacencyMatrixErrorType::InvalidJson(
            ...
            | _ => Err(AdjacencyMatrixError(
              AdjacencyMatrixErrorType::InvalidJson(String::from(
                "expected a json array with the matrix array inside it",
              )),
            )),
          }
        },
