...
impl<T: Num> Matrix<T> {
    ...
    pub fn transpose_in_place(&mut self) {
        ...
        let (rows, cols) = (
            self.0.len(),
            self.0.first().map(Vec::len).unwrap_or_default(),
        );
...
pub fn transpose_buf<T, R>(Matrix(matrix): &Matrix<T>) -> Vec<Vec<R>> {
    iter::once((
        matrix.len(),
        matrix.first().map(Vec::len).unwrap_or_default(),
    ))
    .map(|(rows, cols)| {
        iter::once(Vec::with_capacity(cols))
            .map(|mut out| (out.resize_with(cols, || Vec::with_capacity(rows)), out).1)
            .next()
            .unwrap()
    })
    .next()
